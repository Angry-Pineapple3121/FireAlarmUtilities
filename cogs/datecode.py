...
class DateCode(commands.Cog):
    print('★ Loaded cog: Date Code Decoder')
    ...
    async def dcd(
        self,
        ctx,
        brand: Option(str, 'The brand of the device you want to view.', choices=['Simplex', 'Loganetics'], required=True),
        datecode: Option(str, 'The date code of the device. Format: "22 107 V" (Simplex) or "24241A" (Loganetics).', required=True),
    ):
        """Checks when a Simplex device was manufactured based on its date code."""
        requestTime = datetime.now()
        print(f'[{requestTime}] [DCD] Requested by {ctx.author} (ID: {ctx.author.id}) with date code {datecode}')
        ...
        if ctx.author.id in GLOBAL_BLACKLIST:
            ...
            # Loganetics date code decoder
            else:
                try:
                    embed = discord.Embed(
                        title=f"<:slash:1150933397179486339> Loganetics Date Code Decoder",
                        description=f'Finished decoding date code **{datecode}**.',
                    ...
                    # regex for our date code
                    code_regex = '[0-9]{2}[0-9]{3}([A-Z a-z]{1})?'
                    ...
                    try:
                        ...
                        await ctx.respond('<:warn:1105998033335898162> The date code you entered does not follow the Loganetics date code format.', ephemeral=True)
                        return
                    # if no error match again and do the stuff
                    else:
                        date_code = re.match(code_regex, datecode).group()
                        ...
                        embed.add_field(name=f":gear: Manufacturing Date", value=f'**```{readable_date}```**', inline=True)
                        embed.add_field(name=f":hammer: Revision", value=f'**```{rev}```**', inline=True)
                        await ctx.respond(embed=embed)
                except Exception as e:
                    await ctx.respond('<:warn:1105998033335898162> An error occurred when processing your command. Please contact `Angry_Pineapple#6926` with what you were attempting to do, along with the date and time.', ephemeral=True)
                    print(f'[{requestTime}] [*** BOT ERROR ***] {e}')
